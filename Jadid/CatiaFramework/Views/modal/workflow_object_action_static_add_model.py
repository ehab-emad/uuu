...
def static_shared_component_select(request):
    ...
    post_dict = {key: value[0] if isinstance(value, list) else value for key, value in dict(request.POST).items()}
    form = SharedComponentSelectorForm(request.user.projectuser.current_project_id, level = group_level, **post_dict)
    context = {
        'form': form,
    ...
    data = {
        ...
        'html_form': render_to_string('modals/shared_component/static_shared_component_select.html', context, request=request)
    }
    return JsonResponse(data)


def static_shared_component_reload(request):
    ...
    data = {
        'html_category_groups': render_to_string('modals/shared_component/static_shared_component_group_cards.html', context_category_groups, request=request),
        'html_norm_parts_collector': render_to_string('modals/shared_component/static_shared_component_table.html', context, request=request)
    }
    return JsonResponse(data)


def static_shared_component_propagate(request):
    return execute_object_action(request)
