...
def norm_part_quick_position(request):
    '''
    WARNING: Hardcoded workflow selection may lead to problems in future.
    We should somehow incorporate workflows into quick positioning agent.
    However, the question still remains, whether it does make any sense to connect it to workflows topic.
    Because then probably we make it just complicated.
    -> View is completed, now the function has to be created. In order not to completely destroy an application or
        possibly start programming functionality completely from scratch, we might reuse existing application functionalities 
        by simply accessing session object where we would take actions.  
    -> Hardcoding in Code or directly in form definition? Basically, that does not make any difference.
        So basically, this function prepares input for next function and should handle and obtain a number of object from 
        current session
    '''
    ...
    pass
    last_object_id = list(session_object.workflow_status["w.2"]["objects"].keys())[-1]
    object_uuid = session_object.workflow_status["w.2"]["objects"][last_object_id]["uid"]

    new_command = 'object_utilisation'
    ...
    del session_object.workflow_status["w.2"]["objects"][last_object_id]
    session_object.save()
...
def norm_part_select_modal_config(request):
    ...
    query_part = NormParts_Shared_Component.objects.filter(UUID = uuid)
    if query_part:
    ...
    # ToDo: This is perhaps obsolete, buth commands are the same and can be replaced by 
    #   following in comment
    # instance = normpart if request.method == 'POST' else normpart
    # form = NormPartSelectConfigurationForm(instance = instance)
...
def norm_part_select_modal(request, id, uid, wid):
    ...
    form = NormPartSelectConfigurationCollector(norm_parts = norm_parts, ids = ids)
    # here we need to aasign all the information from object, so we have to pass it as input
...
def norm_part_from_template_save_form(request, form, template_name):
    ...
    category_id = "96540593-2cc7-460b-b453-6438078313a4"
    norm_parts = Component_Group_Level.objects.filter(UUID = category_id).get().normparts_shared_components.filter(type="PART")
    context['norm_parts_collector'] = [Configuration(meta = obj) for obj in norm_parts]
    category_group_object = Component_Group_Level.objects.filter(UUID = "96540593-2cc7-460b-b453-6438078313a4").first()
...
def propagate(request, id, uid, wid) -> JsonResponse:
    ...
    logger.info(f'Accessing Function: connect from frame: {__name__} \n Function Task: propagate')   
    async_obj = async_to_sync(channel_layer.group_send)(channel_name,{
        'type': 'framework_command',
        'message': json.dumps({
            'user': user, 
