...
class PdfReport:
    def __init__(self, buffer, analysis_id):
        ...
        query = Analysis_Comparison.objects.filter(pk = self.analysis_id)
        if query:
        ...
        # Report settings from analysis coparison
    ...
    def _header_footer(self, canvas, doc):
        ...
        try:
            owner = ac.owner.reference_projectuser.user.username
        except:
            owner = ''
        footer_data = [[ django_customer_short_name + ' QLCA Report', 'Author: ' + owner, 'Software Version: '+ application_version, 
    ...
    def autoScale(self, max_value, min_value, desired_steps):
        ...
        #multiples = np.array([0.01, 0.02, 0.03, 0.05, 0.10, 0.25, 0.30, 0.50, 1, 2, 3, 5, 10, 20, 25, 30, 50, 100, 200, 250, 300, 500, 1000, 1500, 2000, 2500, 3000, 5000, 10000])
    ...
    def stacked_barchart_maker(self, scale_factor, data, legend_items, categories, unit, desired_steps, decimals = 3):
        ...
        #helper = barchart.add(Rect(0, 0, barchart.width, barchart.height, fillColor = colours.gray))
        ...
        max_data =max(sum(i for i, j in data if i is not None and i >= 0),sum(j for i, j in data if j is not None and j >= 0))
        min_data =min(sum(i for i, j in data if i is not None and i < 0),sum(j for i, j in data if j is not None and j < 0))
        ...
        bc.categoryAxis.categoryNames = [categories[0],categories[1]]
        ...
        try:
            ...
            if bp_left:
                ...
                cleaned_data.extend([tup[1] for tup in data if tup[1] is not None])
        ...
        if any(item[0] is None for item in data) and 'legend_left' in locals():
            ...
            #Legend Right Creation
            additional_spacing = 50
            legend_right = Legend()
            legend_right.alignment = 'right'
            legend_right.x = (3 * bc.x + legend_left.x + additional_spacing) if 3 * bc.x >= legend_left_width else (legend_left.x + legend_left_width + legend_left.deltax + additional_spacing)
            legend_right.y = barchart.height - 2*bc.y + 30
            legend_right.subCols.rpad = 10
            legend_right.colorNamePairs = cNP_right_rev
            legend_right.deltax = 10
            legend_right.columnMaximum = 15
            legend_right.variColumn = 1
            legend_header_right = String(legend_right.x, legend_left.y+bc.y/2, categories[1], fontSize=10, fontName = 'Times-Bold', textAnchor='start')
            barchart.add(legend_right)
            barchart.add(legend_header_right)
        ...
        if 'legend_left' in locals() and 'legend_right' in locals():
            legend_height = max(legend_left._calcHeight(),legend_right._calcHeight())
        elif 'legend_left' in locals():
    ...
    def createReportForAnalysis(self, request):
        # build document, write to buffer and return pdf
        self.reportContent_Analysis(request)
        self.doc.build(self.Story,canvasmaker=CustomCanvas, onFirstPage=self._header_footer, onLaterPages=self._header_footer)
        pdf = self.buffer.getvalue()
        self.buffer.close()
        return pdf
    ...
    def reportContent(self, request):
        ...
        current_analysis_comparison = get_object_or_404(Analysis_Comparison, pk=self.analysis_id)
        ...
        vehicle = reference_concept.vehicles.all()[:1].get()
        ...
        primary_property = current_analysis_comparison.primary_property
        secondary_properties = current_analysis_comparison.secondary_properties
        lca_parts_left_all = analysis_left.lca_part_models.all()
        lca_parts_right_all = analysis_right.lca_part_models.all()
        lca_parts_all = (analysis_left.lca_part_models.all() | analysis_right.lca_part_models.all()).distinct()
        ...
        lca_processes_all = Instance_Idemat_Database_Process.objects.filter(lca_part_instance__in= lca_parts_left_all, is_active= True) | Instance_Idemat_Database_Process.objects.filter(lca_part_instance__in= lca_parts_right_all, is_active= True)
        lca_processes_left = Instance_Idemat_Database_Process.objects.filter(lca_part_instance__in= lca_parts_left_all , is_active= True)
        lca_processes_right = Instance_Idemat_Database_Process.objects.filter(lca_part_instance__in= lca_parts_right_all, is_active= True)

        #define units for weight
        weight_unit =""
        if current_analysis_comparison.analysis_settings.weight_units == "KILOGRAMS":
            weight_unit ="[kg]"
        if current_analysis_comparison.analysis_settings.weight_units == "GRAMS":
            weight_unit ="[g]"

        decimals = current_analysis_comparison.analysis_settings.weight_decimal_points
        ...
        if self.include_goal_definition == True:
            ...
            self.Story[-1].keepWithNext = True
            goal = Paragraph(current_analysis_comparison.goal_definition , styles['Normal'])
        ...
        if self.include_scope_definition == True:
            ...
            self.Story[-1].keepWithNext = True
            scope = Paragraph(current_analysis_comparison.scope_definition, styles['Normal'])
            self.Story.append(scope)

        #Title and Summary Tables and Charts
        if self.include_title_and_summ_pages == True:
            if current_analysis_comparison.analysis_settings.is_automotive:
                #Vehicle Data Header
                self.Story.append(Paragraph('Vehicle Specification', styles['Heading3']))
                self.Story[-1].keepWithNext = True

                #Overview table from objects
                overview_data = [['Vehicle name', vehicle.reference_vehicle.name,'Vehicle class', vehicle.reference_vehicle.get_vehicle_classification_display().split("(")[0]],
                                ['Lifetime [years]', vehicle.reference_vehicle.life_time_in_years,'Lifetime [km]', vehicle.reference_vehicle.life_distance_in_km],
                                ['Target weight [kg]', vehicle.reference_vehicle.target_weight, 'Weight estimation [kg]', vehicle.reference_vehicle.estimated_weight],
            ...
            carbon_footprint_left = 0.00
            carbon_footprint_right = 0.00
            for part in lca_parts_left_all:
                carbon_footprint_left += part.as_dict()['lca_result']['carbon_footprint']
            for part in lca_parts_right_all:
                carbon_footprint_right += part.as_dict()['lca_result']['carbon_footprint']

            weight_concept_left = analysis_left.get_weight_in_units(current_analysis_comparison.analysis_settings.weight_units)
            weight_concept_right = analysis_right.get_weight_in_units(current_analysis_comparison.analysis_settings.weight_units)
            analysis_right.get_weight_in_units(current_analysis_comparison.analysis_settings.weight_units)
            self.Story.append(Paragraph('Comparison of Concepts', styles['Heading3']))
            self.Story[-1].keepWithNext = True
            comparison_data =   [['','Reference Concept','Alternative Concept'],
                                ['Name', analysis_left.name, analysis_right.name],
                                ['Number of different parts', lca_parts_left_all.count(), lca_parts_right_all.count()],
                                ['Total number of parts', sum(part.multiplier for part in lca_parts_left_all), sum(part.multiplier for part in lca_parts_right_all)],
                                ['Weight ' + weight_unit,  f"{weight_concept_left:.{decimals}f}",  f"{weight_concept_right:.{decimals}f}"],
                                ['GWP CO2eq' + '[kg]', f"{carbon_footprint_left:.{decimals}f}",f"{carbon_footprint_right:.{decimals}f}"]
                                ]
            if utilisation is not None and self.include_utilisation == True:
                comparison_data.append(['Emissions CO2-eq [g/km]',"{:g}".format(round(utilisation.engwpperkm_vehicle_for_analysis_left*10,2))+'*',str(round(utilisation.engwpperkm_vehicle_for_analysis_right*10,2))+'*'])
            comparison_table = Table(comparison_data,colWidths=[self.doc.width*0.4,self.doc.width*0.3,self.doc.width*0.3]) #, colWidths=self.doc.width/3
            ...
            #Weight Comparison
            self.Story.append(Paragraph('Weight Comparison', styles['Heading3']))
            self.Story[-1].keepWithNext = True

            #dataprep for stacked barchart
            weight_data = []
            weight_legend_items = []
            weight_data.append((weight_concept_left, None))
            weight_legend_items.append(analysis_left.name)
            weight_data.append((None ,weight_concept_right))
            weight_legend_items.append((analysis_right.name))
            weight_categories = [analysis_left.name,analysis_right.name]

            #barchart maker
            self.stacked_barchart_maker(0.25, weight_data, weight_legend_items, weight_categories, weight_unit, 5, decimals)
        ...
        if self.include_part_list_pages == True:
            ...
            if len(lca_parts_commodity) != 0:
                categories.append('Common')
            if len(lca_parts_left) != 0:
                categories.append('Reference')
            if len(lca_parts_right) != 0:
                categories.append('Alternative')

            header = Table([['Part Name', 'Part Properties', '']],colWidths=[self.doc.width/6,self.doc.width/6,self.doc.width/6])
            ...
            for category in categories:
                if category == 'Common':
                    partlist = lca_parts_commodity
                    datasets = [data_left, data_right]
                    commodity ='(Commodity)'
                    data_left.append(header)
                    data_right.append(header)
                elif category == 'Reference':
                    partlist = lca_parts_left
                    datasets = [data_left]
                    commodity =''
                    data_left.append(header) if len(data_left)==0 else None
                elif category == 'Alternative':
                    partlist = lca_parts_right
                    datasets = [data_right]
                    commodity =''
                    data_right.append(header) if len(data_right)==0 else None
                for dataset in datasets:
                    for part in partlist:
                        data = []
                        weight = part.get_weight_conceptman(units=current_analysis_comparison.analysis_settings.weight_units)
                        ...
                        single_part = Table(data_wrapped,colWidths=[self.doc.width/6,self.doc.width/6,self.doc.width/6])
            ...
            overall_data = [['Reference Concept', 'Alternative Concept']]

            for i in range(max(len(data_left), len(data_right))):
                        sub_list = []
                        if i < len(data_left):
                            ...
                            sub_list.append('')
                        if i < len(data_right):
                            sub_list.append(data_right[i])
                        else:
                            sub_list.append('')
                        overall_data.append(sub_list)

            table = Table(overall_data,colWidths=[self.doc.width/2,self.doc.width/2], repeatRows=2)
        ...
        if self.include_processes_list_pages == True:
            ...
            for phase, phasename in phases:
                if lca_processes_all.filter(lca_step = phase).exists():
                    ...
                    data_right = []
                    for category in categories:
                        header = Table([['Part Name','Process Name', 'Process Properties', '']],colWidths=[self.doc.width/8,self.doc.width/8,self.doc.width/8,self.doc.width/8])
                        header.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'LEFT'),
                                                    ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
                                                    ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'),
                                                    ('LINEBELOW', (0,0), (-1,0), 1.5, colours.black)]))
                        if category == 'Reference':
                            phase_processes =lca_processes_left.filter(lca_step = phase).order_by('lca_part_instance__created_at').all()
                            data_left.append(header)
                        elif category == 'Alternative':
                            phase_processes =lca_processes_right.filter(lca_step = phase).order_by('lca_part_instance__created_at').all()
                            data_right.append(header)
                        if phase_processes.exists():
                            for process in phase_processes:
                                ...
                                #Flags will be deactivated until correctly implemented in the backend
                                ...
                                s.wordWrap = 'CJK'
                                data_wrapped = [[Paragraph(cell, s) for cell in row] for row in data]
                                single_process = Table(data_wrapped,colWidths=[self.doc.width/8,self.doc.width/8,self.doc.width/8,self.doc.width/8])
                                single_process.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'LEFT'),
                                                                    ...
                                                                    ('LINEBELOW', (0,-1), (-1,-1), 0.25, colours.black)]))
                                data_left.append(single_process) if category == 'Reference' else None
                                data_right.append(single_process) if category == 'Alternative' else None
        ...
        weight_data = []
        weight_legend_items = []
        for part in lca_parts_left_all.order_by('created_at'):
            weight = part.get_weight_conceptman(units=current_analysis_comparison.analysis_settings.weight_units) * part.multiplier
            weight_data.append((weight , None))
            weight_legend_items.append(str(part.multiplier) + ' x ' + part.part_model.name  + ' = ')
        for part in lca_parts_right_all.order_by('created_at'):
            weight = part.get_weight_conceptman(units=current_analysis_comparison.analysis_settings.weight_units) * part.multiplier
            weight_data.append((None ,weight))
            weight_legend_items.append(str(part.multiplier) + ' x ' + part.part_model.name  + ' = ')
        weight_categories = [analysis_left.name,analysis_right.name]


        #barchart maker
        self.stacked_barchart_maker(0.4, weight_data, weight_legend_items, weight_categories,weight_unit, 5, decimals)
        ...
        gwp_legend_items = []
        for part in lca_parts_left_all.order_by('created_at'):
            gwp_data.append((part.as_dict()['lca_result'][primary_property.name], None))
            gwp_legend_items.append(str(part.multiplier) + ' x ' + part.part_model.name + ' = ')
        for part in lca_parts_right_all.order_by('created_at'):
            gwp_data.append((None ,part.as_dict()['lca_result'][primary_property.name]))
            gwp_legend_items.append(str(part.multiplier) + ' x ' + part.part_model.name + ' = ')
        gwp_categories = [analysis_left.name,analysis_right.name]
        #barchart maker
        ...
        gwp_data = []
        gwp_legend_items = []
        for process in lca_processes_left.order_by('lca_part_instance__created_at'):
            part = lca_parts_all.filter(lca_process_model = process).get()
            gwp_data.append((getattr(process.results_model, primary_property.name)*part.multiplier, None))
            gwp_legend_items.append(process.name)
        for process in lca_processes_right.order_by('lca_part_instance__created_at'):
            part = lca_parts_all.filter(lca_process_model = process).get()
            gwp_data.append((None, getattr(process.results_model, primary_property.name)*part.multiplier))
            gwp_legend_items.append(process.name)
        gwp_categories = [analysis_left.name,analysis_right.name]
