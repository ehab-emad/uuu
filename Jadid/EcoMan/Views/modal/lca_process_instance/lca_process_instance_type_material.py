...
from EcoMan.models import Instance_Idemat_Database_Process, Analysis, Analysis_Comparison
def material_process_instance_update(request, pk_instance,  weight_unit = None, weight_decimals = 3):
    '''
    ...
    if request.method == 'POST':
        #form will recalculate grams to kilograms
        form = MaterialProcessInstanceUpdateForm(request.POST, instance=instance_lca_process, weight_unit = weight_unit , weight_decimals = weight_decimals)
    if request.method == 'GET':  
        ...
        form = MaterialProcessInstanceUpdateForm(instance=instance_lca_process, initial = initial_dict, weight_unit= weight_unit, weight_decimals = weight_decimals)
    html_template = 'modals/instance_idemat_process/material_instance_idemat_process_update_modal.html'
    return save_form(request, form, html_template, weight_unit, weight_decimals)


def save_form(request, form, template_name,  weight_unit ='KILOGRAMS', weight_decimals = 3):
    data = dict()
    context = dict()
    if request.method == 'POST':
        if form.is_valid():
