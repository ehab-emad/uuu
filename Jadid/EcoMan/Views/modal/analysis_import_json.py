...
from EcoMan.models import Analysis, Analysis, Analysis_Settings, ProjectUser_EcoMan_Ref
from ConceptMan.models import Concept
from django.urls import reverse
import json

def create_new_analysis(name,project_model, user, json_file):

   analysis = Analysis.objects.create(name=name, project_model=project_model)

   #concept_left
   concept= Concept.objects.create(project_model=project_model)
   concept.save()
   concept.vehicles.add(user.projectuser.sandbox_vehicle)
   concept.save()

   #analysis_left
   analysis= Analysis()
   analysis.owner = user
   analysis.project_model = project_model
   analysis.concept_model = concept
   analysis.save()
   ...
   if request.method == 'POST':
      if form.is_valid():
         ...
         try:
            ...
            analysis = create_new_analysis(name=form.cleaned_data['name'], project_model=proj_model , projectuser=request.user.projectuser)

            json_file = form.cleaned_data['JSON_file']      
            #concept_left
            concept= Concept.objects.create(project_model=proj_model)
            concept.save()
            concept.vehicles.add(request.user.projectuser.sandbox_vehicle)
            concept.save()
            ...
            analysis.project_model = proj_model
            analysis.concept_model = concept
            analysis.save()
   ...
   if request.method == 'POST':
      if form.is_valid():
         ...
         if request.FILES:
               ...
               analysis.import_json(json_data)
      else:
         data['form_is_valid'] = False
...
def json_load_process_comparison(json_data):
   ...
   for part in json_data['analysis_left']['parts'].values():
   ...
   for part in json_data['analysis_right']['parts'].values():
      pass
...
def json_import_analysis_comparison_append(request, pk_analysis):
