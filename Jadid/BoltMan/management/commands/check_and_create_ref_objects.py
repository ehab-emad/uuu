...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        '''
        from BoltMan.models import Project_BoltMan_Ref
        from BoltMan.models import ProjectUser_BoltMan_Ref
        from BoltMan.models import Vehicle_BoltMan_Ref        
        ...
        for object in query_projects:
            project_ref = Project_BoltMan_Ref.objects.filter(UUID = str(object.UUID))
            if not project_ref:
                        Project_BoltMan_Ref.objects.create(UUID = str(object.UUID))                                                    

        for object in query_projectsusers:
            projectuser_ref = ProjectUser_BoltMan_Ref.objects.filter(UUID = str(object.UUID))
            if not projectuser_ref:
                        ProjectUser_BoltMan_Ref.objects.create(UUID = str(object.UUID))     

        for object in query_vehicle:
            vehicle_ref = Vehicle_BoltMan_Ref.objects.filter(UUID = str(object.UUID))
            if not projectuser_ref:
                        Vehicle_BoltMan_Ref.objects.create(UUID = str(object.UUID))     
